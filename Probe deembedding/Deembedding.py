...
Stimulus, MS11O = acq('MS11O.CSV', folder, deliminput)
Stimulus, MS11L = acq('MS11L.CSV', folder, deliminput)

if probe_flag == 2:
    Stimulus, MS22S = acq('MS22S.CSV', folder, deliminput)
    Stimulus, MS22O = acq('MS22O.CSV', folder, deliminput)
    Stimulus, MS22L = acq('MS22L.CSV', folder, deliminput)
...
# S21 = S12 = +/-(z + x * y)**0.5; the sign will be checked based on the slope of the unwrapped phase
x = np.array(x)
y = np.array(y)
z = np.array(z)

Trans = (z + x * y)**0.5
realTrans = Trans.real
imagTrans = Trans.imag
...
# Choosing the proper sign of the transmission coefficients: unwrapped phase must be negative
if slopesign >= 0:
#   Trans = -(z + x * y) ** 0.5
    realTrans = -realTrans
    imagTrans = -imagTrans

# S2P matrix of the probe
...
if probe_flag == 2 and VNA == 0:
    ...
    Stimulus, MS11 = acq('MS11.CSV', folder, deliminput)
    Stimulus, MS22 = acq('MS22.CSV', folder, deliminput)
    Stimulus, MS21 = acq('MS21.CSV', folder, deliminput)
    Stimulus, MS12 = acq('MS12.CSV', folder, deliminput)
    MS11 = np.array(MS11)
    MS22 = np.array(MS22)
    MS21 = np.array(MS21)
    MS12 = np.array(MS12)
